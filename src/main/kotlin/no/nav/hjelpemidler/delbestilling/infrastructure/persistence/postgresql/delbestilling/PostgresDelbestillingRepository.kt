...
class PostgresDelbestillingRepository(val tx: JdbcOperations) : DelbestillingRepository {
    ...
    override fun hentDelbestillinger(bestillerFnr: String): List<DelbestillingSak> = tx.list(
        sql = """
            SELECT * 
            FROM delbestilling
    ...
    override fun hentDelbestillingerForKommune(brukerKommunenr: String): List<DelbestillingSak> = tx.list(
        sql = """
            SELECT * 
            FROM delbestilling
    ...
    override fun hentDelbestillinger(hmsnr: Hmsnr, serienr: Serienr): List<DelbestillingSak> = tx.list(
        sql = """
            SELECT * 
            FROM delbestilling
            WHERE
    ...
    override fun hentDelbestilling(saksnummer: Long): DelbestillingSak? = tx.singleOrNull(
        sql = """
            SELECT * 
            FROM delbestilling
    ...
    override fun hentDelbestilling(oebsOrdrenummer: String): DelbestillingSak? = tx.singleOrNull(
        sql = """
            SELECT * 
            FROM delbestilling
